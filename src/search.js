...
async function search(collection, searchString, params = {}) {
	...
	if (params.limit > config.MAX_LIMIT) {
		params.limit = config.MAX_LIMIT
	}
	...
	const aggregate = [{
		...
		$project: _.extend({}, {...params.projection, _id: 1}, {
			score: {
				$meta: 'textScore'
			}
		})
	...
	if (fullPageOfResults) {
		response.next = bsonUrlEncoding.encode([_.last(results).score, _.last(results)._id])
	}
	return response
}
