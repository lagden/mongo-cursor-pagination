...
module.exports = {
	...
	prepareResponse(results, params) {
		...
		const hasPrevious = Boolean(params.next) || Boolean(params.previous && hasMore)
		const hasNext = Boolean(params.previous) || hasMore
		...
		if (response.previous) {
			...
			response.previous = shouldSecondarySortOnId ? bsonUrlEncoding.encode([previousPaginatedField, response.previous._id]) : bsonUrlEncoding.encode(previousPaginatedField)
		}

		if (response.next) {
			const nextPaginatedField = objectPath.get(response.next, params.paginatedField)
			response.next = shouldSecondarySortOnId ? bsonUrlEncoding.encode([nextPaginatedField, response.next._id]) : bsonUrlEncoding.encode(nextPaginatedField)
		}
